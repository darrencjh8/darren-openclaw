...
function makeBudget(overrides = {}) {
  return { name: overrides.name || "TestBudget", groupId: "g-1", ...overrides };
}
...
describe("init", () => {
  ...
  test("first call initializes Actual and downloads the target budget", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("g-1", { password: undefined });
  });
  ...
  test("concurrent calls return same promise — no double init", async () => {
    const api = actual();
    let resolveInit;
    api.init.mockReturnValue(new Promise((r) => { resolveInit = r; }));
    api.getBudgets.mockResolvedValue([makeBudget()]);
    api.downloadBudget.mockResolvedValue(undefined);
  ...
  test("finds budget by BUDGET_FILE name", async () => {
    ...
    api.getBudgets.mockResolvedValue([
      makeBudget({ name: "Darren MYR", groupId: "myr-1" }),
      makeBudget({ name: "Darren SGD", groupId: "sgd-1" }),
    ]);
    api.downloadBudget.mockResolvedValue(undefined);
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("sgd-1", { password: undefined });
  });
  ...
  test("falls back to first budget when name does not match", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("first-1", { password: undefined });
  });
  ...
  test("uses cloudFileId when groupId is absent", async () => {
    const api = actual();
    api.init.mockResolvedValue(undefined);
    api.getBudgets.mockResolvedValue([makeBudget({ groupId: null, cloudFileId: "cloud-xyz" })]);
    api.downloadBudget.mockResolvedValue(undefined);

    const { init } = require("../server");
    await init();

    expect(api.downloadBudget).toHaveBeenCalledWith("cloud-xyz", expect.any(Object));
  });
...
describe("ensureBudget", () => {
  ...
  async function primeInit(budgetName = "SGD") {
    ...
    api.init.mockResolvedValue(undefined);
    api.getBudgets
      .mockResolvedValueOnce([makeBudget({ name: budgetName, groupId: "sgd-sync" })])
      .mockResolvedValue([makeBudget({ name: budgetName, groupId: "sgd-sync" })]);
    api.downloadBudget.mockResolvedValue(undefined);
  ...
  test("matches budget by groupId", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("id-b", expect.any(Object));
  });
  ...
  test("matches budget by cloudFileId", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("cf-b", expect.any(Object));
  });
  ...
  test("matches budget by name", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("b-1", expect.any(Object));
  });
  ...
  test("returns silently when budget not found and no MYR fallback", async () => {
    ...
    api.getBudgets.mockResolvedValue([makeBudget({ name: "SGD", groupId: "sgd-1" })]);
    const { ensureBudget } = loadServer();
  ...
  test("skips download when already on the target budget", async () => {
    await primeInit("SGD");
    const api = actual();
    api.getBudgets.mockResolvedValue([makeBudget({ name: "SGD", groupId: "sgd-sync" })]);
    const { ensureBudget } = loadServer();
  ...
  test("downloads new budget and switches activeSyncId", async () => {
    ...
    expect(api.downloadBudget).toHaveBeenCalledWith("myr-sync", expect.any(Object));
  });

  test("uses cached budget without re-downloading on second switch", async () => {
    await primeInit("SGD");
    const api = actual();
    api.getBudgets.mockResolvedValue([
      makeBudget({ name: "SGD", groupId: "sgd-sync" }),
  ...
  test("MYR fallback via exact MYR_BUDGET_FILE name", async () => {
    ...
    await primeInit("Darren SGD");
    const api = actual();
    api.getBudgets.mockResolvedValue([
      makeBudget({ name: "Darren SGD", groupId: "sgd-sync" }),
      makeBudget({ name: "Darren MYR", groupId: "myr-sync" }),
    ]);
    const { ensureBudget } = loadServer();
    await ensureBudget("Darren MYR");
    expect(api.downloadBudget).toHaveBeenCalledWith("myr-sync", expect.any(Object));
  });
