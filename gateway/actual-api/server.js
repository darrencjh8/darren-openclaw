...
const SERVER_URL = process.env.ACTUAL_BUDGET_SERVER_URL || process.env.ACTUAL_BUDGET_URL;
...
const BUDGET_SWITCH_DELAY_MS = parseInt(process.env.BUDGET_SWITCH_DELAY_MS || "2000", 10);
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isNetworkError =
        e.message && (e.message.includes("network") ||
...
async function init() {
  ...
  initPromise = (async () => {
    await retryWithBackoff(() =>
      actual.init({ serverURL: SERVER_URL, password: PASSWORD, dataDir: DATA_DIR })
    );
...
app.get("/budget-12m", async (req, res) => {
  try {
    await ensureBudget(getBudgetId(req));
    const now = new Date();
    let total12m = 0, emergency = 0, invest = 0;
    ...
    try {
      ...
      for (const g of curData.categoryGroups || [])
        for (const c of g.categories || []) {
          ...
          if (c.name === 'General Investment') invest = c.balance || 0;
        }
    } catch(e) { /* ignore */ }
