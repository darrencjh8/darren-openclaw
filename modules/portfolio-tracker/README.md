...
```bash
pytest tests/ -v          # 154 tests (Python)
mvn test -f pp-cli        # Java CLI tests: 19 tests
```

Total: 154 (Python) + 19 (Java) = 173 tests.

## Feature Breakdown

`.speckit/features/` catalogs every feature with a `plan.md`, `spec.md`, and `tasks.md`:

| Feature directory | Status |
|---|---|
| `ibkr-import/` | Implemented — IBKR flex query XML parser (Trades, CashTransactions, CorporateActions) |
| `balance-sync/` | Implemented — AB → PP sync with OneDrive auto pull/push |
| `taxonomy-export/` | Implemented — Taxonomy export to Google Sheets |
| `cron-automation/` | Implemented — Cron scheduling (pp-sync-all daily at 3 AM SGT via apscheduler) |
| `cpf-sync/` | Spec written, not implemented — CPF statement PDF → PP + Actual Budget |
| `poems-sync/` | Spec written, not implemented — POEMS statement PDF → PP |

See `.speckit/AUDIT.md` for test status, known bugs, and feature completion details.
