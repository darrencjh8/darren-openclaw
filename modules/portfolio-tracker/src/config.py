...
class Config:
    ...
    google_sheet_id: str = ""
    taxonomy_names: list[str] = field(
        default_factory=lambda: ["Sector", "Geography", "Asset Class"]
    )
    taxonomy_sheet_mapping: dict[str, str] = field(default_factory=dict)
    ...
    log_level: str = "INFO"
    pp_sync_all_cron: str = "0 3 * * *"
    balance_sync_model: str = ""  # override LLM model for balance sync (e.g. deepseek-v4-pro)
    user_name: str = "there"
    ...
    imap_username: str = ""
    imap_password: str = ""
    imap_folder: str = "Trades"
    openclaw_gateway_url: str = "http://openclaw:18800"
    notification_smtp_host: str = "smtp.example.com"
    ...
    def from_env(cls) -> "Config":
        ...
        mapping_raw = os.environ.get("TAXONOMY_SHEET_MAPPING", "")
        taxonomy_sheet_mapping: dict[str, str] = {}
        if mapping_raw:
            for pair in mapping_raw.split(","):
                if ":" in pair:
                    key, cell = pair.split(":", 1)
                    taxonomy_sheet_mapping[key.strip()] = cell.strip()
        ...
        return cls(
            ...
            pp_sync_all_cron=os.environ.get("PP_SYNC_ALL_CRON", "0 3 * * *"),
            balance_sync_model=os.environ.get("BALANCE_SYNC_MODEL", ""),
            ...
            imap_folder=os.environ.get("IMAP_FOLDER", "Trades"),
            openclaw_gateway_url=os.environ.get("OPENCLAW_GATEWAY_URL", "http://openclaw:18800"),
            notification_smtp_host=os.environ.get("NOTIFICATION_SMTP_HOST", "smtp.example.com"),
