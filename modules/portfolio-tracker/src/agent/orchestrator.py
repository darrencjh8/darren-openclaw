...
class DeepSeekClient:
    def __init__(self, api_key: str, balance_sync_model: str = ""):
        self._client = AsyncOpenAI(
            api_key=api_key,
            base_url="https://api.deepseek.com/v1",
            timeout=30.0,
        )
        self._balance_sync_model = balance_sync_model
    ...
    async def chat(
        ...
        for attempt in range(3):
            try:
                ...
                if usage:
                    logger.info(
                        "DeepSeek API: model=%s attempt=%d input=%d output=%d",
                        model,
                        attempt + 1,
                        usage.prompt_tokens,
                ...
                if attempt < 2:
                    wait = 2**attempt
                    logger.warning(
                        "DeepSeek API attempt %d failed: %s — retrying in %ds", attempt + 1, e, wait
                    )
                    time.sleep(wait)
...
class AgentOrchestrator:
    ...
    async def process_event(
        ...
        if event_type == "email_trade":
            self._tools.set_event_context(
                raw_email=data if isinstance(data, bytes) else b"",
            )

        messages = self._build_messages(event_type, data)
        return await self._run_loop(messages, event_type)

    async def _run_loop(self, messages: list[dict], event_type: str = "") -> dict:
        override_model = self._llm._balance_sync_model if event_type == "balance_sync" else ""
        for iteration in range(7):
            ...
            if message.tool_calls:
                ...
                for tc in message.tool_calls:
                    ...
                    if isinstance(result_parsed, dict) and result_parsed.get(
                        "requires_confirmation"
                    ):
                        return await self._handle_confirmation(messages, tc, result_parsed)
    ...
    async def _handle_confirmation(self, messages: list[dict], tc, result: dict) -> dict:
        ...
        # Send confirmation prompt via gateway webhook
    ...
    def _build_messages(self, event_type: str, data: bytes | str) -> list[dict]:
        ...
        if event_type == "ibkr_flex_query":
            ...
            user_msg = (
                "Run the daily balance sync: fetch Actual Budget categories and update PP balances."
            )
