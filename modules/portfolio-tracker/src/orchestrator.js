...
export class DeepSeekClient {
  ...
  async chat(messages, tools) {
    const kwargs = { model: this._model, messages, temperature: 0.1 };
...
export class AgentOrchestrator {
  ...
  async processEmail(msgId, rawEmail, imapHandler) {
    const emailText = Buffer.isBuffer(rawEmail) ? rawEmail.toString('utf8') : String(rawEmail);
    ...
    for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
      ...
      const amsg = { role: 'assistant', content: message.content, tool_calls: toolCalls };
      ...
      for (const tc of toolCalls) {
        ...
        const result = await this._tools.executeTool(func.name || '', args);
        messages.push({
  ...
  _buildMessages(emailContent) {
    ...
    messages.push({ role: 'user', content: `Process this:\n\n${emailContent}` });
    return messages;
  }
}
