...
class PpJavaBridge:
    ...
    async def _run_command(self, *args: str) -> dict:
        ...
        try:
            ...
            if proc.returncode != 0:
                ...
                if (
                    "IllegalBlockSize" in error_msg
                    or "decrypt" in error_msg
                    or "corrupt" in error_msg.lower()
                ):
                    try:
                        await self._sync_from_onedrive()
                        ...
                        stdout2, stderr2 = await asyncio.wait_for(
                            proc2.communicate(), timeout=self._timeout
                        )
                        if proc2.returncode != 0:
                            raise RuntimeError(stderr2.decode("utf-8", errors="replace").strip())
                        return json.loads(stdout2.decode("utf-8", errors="replace"))
                    except Exception:
    ...
    async def pull(self) -> dict:
        """Download latest PP file from OneDrive. Call before viewing/editing."""
        try:
            proc = await asyncio.create_subprocess_exec(
                "python3",
                "/app/src/onedrive_download.py",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            ...
            return {"status": "error", "detail": stderr.decode().strip()}
        except Exception as e:
            return {"status": "error", "detail": str(e)}

    async def push(self) -> dict:
        """Upload PP file to OneDrive. Call after making changes."""
        try:
            proc = await asyncio.create_subprocess_exec(
                "python3",
                "/app/src/onedrive_upload.py",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=30)
