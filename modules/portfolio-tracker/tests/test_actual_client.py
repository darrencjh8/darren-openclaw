...
async def test_find_budget_by_name(ab_client):
    ab_client.list_budgets = AsyncMock(return_value=[{"id": "budget-1", "name": "Darren SGD"}])
    result = await ab_client.find_budget("Darren SGD")
    assert result["id"] == "budget-1"


@pytest.mark.asyncio
async def test_get_categories_budget_not_found_raises(ab_client):
    ab_client.list_budgets = AsyncMock(return_value=[])
    with pytest.raises(ValueError, match="Budget not found"):
        await ab_client.get_categories("nonexistent")
