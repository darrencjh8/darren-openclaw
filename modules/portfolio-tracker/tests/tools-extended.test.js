...
function createMockBridge(responses = {}) {
  return {
    ...
    insertTransaction: vi.fn().mockResolvedValue(responses.insert || { status: "inserted" }),
    updateBalance: vi.fn().mockResolvedValue(responses.balance || { status: "updated" }),
    pull: vi.fn().mockResolvedValue(responses.pull || { status: "ok", detail: "downloaded" }),
    push: vi.fn().mockResolvedValue(responses.push || { status: "ok", detail: "uploaded" }),
    queryTaxonomies: vi.fn().mockResolvedValue(responses.taxonomies || { taxonomies: [] }),
    getTransactions: vi.fn().mockResolvedValue(responses.transactions || []),
    ...
    querySecurity: vi.fn().mockResolvedValue(
      responses.query || { ticker: "AAPL", shares: 100, price: 185.30 },
    ),
  };
}
...
describe("ToolRegistry — dedup & memory tools", () => {
  ...
  beforeEach(() => {
    ...
    const memPath = join(tmpdir(), `test-tools-memory-${crypto.randomUUID()}.json`);
  ...
  describe("check_duplicate", () => {
    ...
    it("returns is_duplicate: true after recording", async () => {
      dedup.record("2026-06-01", 1853000, "acct-1", "corr-1", "sec-aapl", "Buy");
      const result = await registry.executeTool("check_duplicate", {
        date: "2026-06-01",
        amount_cents: 1853000,
        account_id: "acct-1",
...
describe("ToolRegistry — PP bridge tools", () => {
  ...
  beforeEach(() => {
    cfg = new Config(REQUIRED_ENV);
    const dbPath = join(tmpdir(), `test-ppbridge-dedup-${crypto.randomUUID()}.db`);
    dedup = new DedupJournal(dbPath);
    const memPath = join(tmpdir(), `test-ppbridge-memory-${crypto.randomUUID()}.json`);
    memory = new MemoryStore(memPath);
  ...
  describe("query_pp_security", () => {
    it("calls bridge.querySecurity with search term", async () => {
      const result = await registry.executeTool("query_pp_security", { search: "AAPL" });
      expect(mockBridge.querySecurity).toHaveBeenCalledWith("AAPL");
      expect(result).toEqual({ ticker: "AAPL", shares: 100, price: 185.30 });
    });

    it("returns error when bridge not configured", async () => {
      const reg = new ToolRegistry(cfg, dedup, memory, null, null);
      const result = await reg.executeTool("query_pp_security", { search: "AAPL" });
      expect(result).toEqual({ error: "PP bridge not configured" });
    });
  ...
  describe("fetch_pp_accounts", () => {
    it("calls bridge.getAccounts", async () => {
      mockBridge.getAccounts.mockResolvedValue([{ id: "acct-1", name: "IBKR USD" }]);
  ...
  describe("notify_user", () => {
    it("returns status: sent on success", async () => {
      ...
      const result = await registry.executeTool("notify_user", { message: "Hello" });
