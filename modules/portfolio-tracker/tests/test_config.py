...
def test_config_loads_from_env(monkeypatch):
    ...
    monkeypatch.setenv("ACTUAL_BUDGET_FILE", "Test-SGD-Budget")
    monkeypatch.setenv("MYR_BUDGET_FILE", "Test-MYR")
...
def test_config_defaults(monkeypatch):
    ...
    monkeypatch.setenv("ACTUAL_BUDGET_FILE", "Test-SGD-Budget")
    monkeypatch.setenv("MYR_BUDGET_FILE", "Test-MYR")
    monkeypatch.setenv("TAXONOMY_NAMES", "Sector,Geography,Asset Class")
    config = Config.from_env()
...
def test_config_gateway_url_fields(monkeypatch):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "sk-test")
    monkeypatch.setenv("ACTUAL_BUDGET_URL", "https://ab.example.com")
    monkeypatch.setenv("ACTUAL_BUDGET_PASSWORD", "pw")
    monkeypatch.setenv("ACTUAL_BUDGET_FILE", "Test-SGD-Budget")
    ...
    monkeypatch.setenv("IMAP_FOLDER", "CustomInbox")
    config = Config.from_env()
    assert config.openclaw_gateway_url == "http://gateway:9999"
    assert config.imap_folder == "CustomInbox"
