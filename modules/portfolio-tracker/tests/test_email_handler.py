...
async def test_fetch_unread_returns_structured_data(email_handler):
    ...
    mock_imap.fetch = AsyncMock(return_value=("OK", [
        (b"1 (RFC822 {100}", b"From: test@test.com\r\n\r\nBody"),
        b")"
    ]))
    email_handler._imap = mock_imap
...
@pytest.mark.asyncio
async def test_constructor_with_notify_callback():
    cb = AsyncMock()
    handler = EmailHandler("h", 993, "u", "p", MagicMock(), notify_callback=cb)
    assert handler._notify is cb
