...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            self._conn.row_factory = sqlite3.Row
            # Retry-with-jitter for init-time lock contention.  When multiple
            # processes (gateway + CLI sessions + kanban workers) share one
            # state.db, the schema-init phase (PRAGMA journal_mode=WAL, DDL,
            # FTS5 setup) contends with writes from other processes.  The
            # same jitter strategy as _execute_write breaks the convoy instead
            # of failing immediately with "database is locked".
            _init_lock_err: Optional[Exception] = None
            for _init_attempt in range(self._WRITE_MAX_RETRIES):
                try:
                    ...
                    _err_msg = str(_exc).lower()
                    if "locked" in _err_msg or "busy" in _err_msg:
                        _init_lock_err = _exc
                        if _init_attempt < self._WRITE_MAX_RETRIES - 1:
                ...
                # Retries exhausted.
                raise _init_lock_err or sqlite3.OperationalError(
