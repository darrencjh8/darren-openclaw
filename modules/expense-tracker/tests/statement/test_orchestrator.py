...
class TestStatementProcessorProcessStatement:
    ...
    async def test_process_statement_stop_no_tool_calls(self, mock_tools):
        ...
        processor._llm.chat = AsyncMock(return_value={
            "choices": [{
                "finish_reason": "stop",
        ...
        with patch("src.extractors.extract_email_content", return_value="Statement body content here."):
    ...
    async def test_process_statement_with_tool_calls(self, mock_tools):
        processor = self._make_processor(mock_tools)
        processor._llm.chat = AsyncMock(side_effect=[
            {
                "choices": [{
                    ...
                    "message": {
                        ...
                        "tool_calls": [{
                            "id": "call-1",
                            "function": {"name": "fetch_accounts", "arguments": '{"budget_id": "sgd"}'},
                        }],
                    },
                }],
            },
            {
                "choices": [{
                    "finish_reason": "stop",
                    "message": {"content": "Reconciliation complete: 5 matched, 2 outliers."},
                }],
            },
        ])
