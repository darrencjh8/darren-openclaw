...
class TestStatementProcessor:
    ...
    async def test_process_statement_happy_path_2_matched_1_outlier(self):
                    ...
                    "choices": [
                        {
                            ...
                            "message": {
                                ...
                                "tool_calls": [
                                    ...
                                    {
                                        "id": "c2",
                                        "function": {"name": "fetch_categories", "arguments": "{}"},
                                    },
                                    {
                                        "id": "c3",
                                        "function": {
        ...
        tools._handle_fetch_accounts = AsyncMock(return_value=[{"id": "acct-1", "name": "DBS Yuu"}])
        tools._handle_fetch_categories = AsyncMock(return_value=[{"id": "cat-1", "name": "Food"}])
        tools._handle_fetch_statement_history = AsyncMock(return_value=None)
        tools._handle_fetch_unreconciled_transactions = AsyncMock(
            return_value=[
                {
                    "id": "txn-001",
                    "date": "2026-05-04",
    ...
    async def test_process_statement_duplicate_period_stops(self):
        ...
        tools._handle_fetch_statement_history = AsyncMock(
            return_value={
                "id": 1,
                "account_id": "acct-1",
    ...
    def test_llm_client_uses_v4_pro_model(self):
        ...
        from src.agent.tools import ToolRegistry
        from src.statement.orchestrator import DeepSeekClient, StatementProcessor
