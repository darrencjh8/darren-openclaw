...
describe('ToolRegistry', () => {
  ...
  it('executes known tool', async () => {
    const cfg = new Config(testEnv);
    const registry = new ToolRegistry(cfg);
    const result = await registry.executeTool('log_decision', { action: 'test', reasoning: 'unit test' });
  ...
  it('throws on unknown tool', async () => {
    const cfg = new Config(testEnv);
    const registry = new ToolRegistry(cfg);
    await expect(registry.executeTool('nonexistent', {})).rejects.toThrow('Unknown tool');
  });

  it('search_memory returns empty with no memory', async () => {
    const cfg = new Config(testEnv);
    const registry = new ToolRegistry(cfg);
    const result = await registry.executeTool('search_memory', { query: 'test' });
  ...
  it('learn_fact returns no-store with no memory', async () => {
    const cfg = new Config(testEnv);
    const registry = new ToolRegistry(cfg);
    const result = await registry.executeTool('learn_fact', { fact: 'test' });
