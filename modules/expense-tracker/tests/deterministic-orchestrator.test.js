...
describe("Phase 1a: LLM Extract", () => {
    ...
    it("returns {skip: true} for promotional email → detected as non-transaction", async () => {
        ...
        const result = await orch._runPhase1a(
        ...
        const tools = makeTools({
            ...
            executeTool: vi.fn(async (name) => {
                ...
                if (name === "fetch_context")
                    return {
                        accounts: [
                            { id: "acc-1", name: "DBS Yuu", closed: false },
                        ],
...
describe("Phase 3: Web Search + V3 Gate", () => {
    ...
    it("does not set payee when resolve_merchant returns Misc", async () => {
        const { AgentOrchestrator } = await import("../src/orchestrator.js");
        const config = makeConfig();
        const tools = makeTools({
            executeTool: vi.fn(async (name) => {
        ...
        // Misc is not set as payee_name (only set when !== "Misc")
    ...
    it("handles resolve_merchant throwing gracefully", async () => {
        ...
        const tools = makeTools({
            executeTool: vi.fn(async (name) => {
                if (name === "resolve_merchant")
                    throw new Error("Brave API down");
                if (name === "fetch_context")
                    return { accounts: [], categories: [], payees: [] };
                return true;
            }),
...
describe("Full 4-phase flow", () => {
        ...
        const tools = makeTools({
            ...
            executeTool: vi.fn(async (name) => {
                ...
                if (name === "fetch_context")
                    return {
                        accounts: [
                            { id: "acc-dbs", name: "DBS Yuu", closed: false },
                        ],
                        categories: [{ id: "cat-food", name: "Food & Dining" }],
                        payees: [{ name: "Toast Box" }],
                    };
        ...
        orch._llm.chat = vi
            ...
            .mockResolvedValue(
                mockChatResponse({
                    ...
                    notify_message:
                        "Couldn't match an account for Mystery Merchant",
                }),
            );
    ...
    it("Phase 3 exhausted → notifies user", async () => {
        const { AgentOrchestrator } = await import("../src/orchestrator.js");
        const config = makeConfig();
        const tools = makeTools({
            getPhase2ToolSchemas: vi.fn(() => []),
        ...
        orch._llm.chat = vi
            ...
            .mockResolvedValueOnce(
                mockChatResponse({
                    ...
                    notify_message:
                        "Couldn't classify TotallyUnknownVendor. Please categorize manually.",
                }),
            );
        ...
        expect(result.action).toBe("notified");
        expect(result.details).toContain("Phase 3 exhausted");
        expect(tools.executeTool).toHaveBeenCalledWith(
            "notify_user",
            expect.objectContaining({
                message: expect.stringContaining("TotallyUnknownVendor"),
            }),
        );
    });
