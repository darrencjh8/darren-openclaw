...
These tests connect to real Zoho Mail IMAP/SMTP servers using credentials
...
@pytest.mark.asyncio
class TestImapLive:
    """Live IMAP tests against the configured Zoho Mail server."""
...
class TestOrchestratorWiring:
    ...
    async def test_full_pipeline_wired(self, env):
        ...
        handler = ImapIdleHandler(
            env["imap_host"], env["imap_port"],
            env["imap_username"], env["imap_password"],
        )
        await handler.connect()
        ...
        try:
            ...
            config = Config(
                ...
                telegram_bot_token=os.environ.get("TELEGRAM_BOT_TOKEN", "123:test"),
