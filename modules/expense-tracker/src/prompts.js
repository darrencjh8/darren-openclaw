...
/**
 * Phase 1: LLM Analysis prompt (3-phase design).
...
WORKFLOW (follow in EXACT order):
 ...
 3. resolve_merchant(merchant) — get canonical payee. This handles memory
    lookup and web search internally.
...
/**
 * Save a few-shot example for the legacy prompt (backward compat).
 * Unused in the new 3-phase pipeline.
 *
 * @returns {Array}
 */
export function getFewShotExamples() {
  ...
  return [
    ...
    [
      ...
      {
        role: "assistant",
        content: "Web search classified SGSUPERGREEN-B as Misc. Proceeding.",
      },
    ],
