...
async function main() {
  ...
  if (memory.listFacts().length === 0) {
    ...
    if (existsSync(mappingsPath)) {
      console.log(JSON.stringify({ event: 'migrating_mappings_to_memory' }));
  ...
  console.log(JSON.stringify({ event: 'memory_initialized', data: { facts: memory.listFacts().length } }));
  ...
  const toolNames = [
    'search_memory', 'learn_fact', 'list_facts', 'update_fact', 'delete_fact',
    'fetch_accounts', 'fetch_categories', 'fetch_payees', 'fetch_recent_transactions',
    'insert_transaction', 'check_duplicate', 'mark_email_read',
  ...
  for (const name of toolNames) {
    app.post(`/tools/${name.replace(/_/g, '-')}`, async (req, res) => {
      try {
        ...
        res.status(e.message?.startsWith('Unknown') ? 404 : 500).json({ error: e.message, code: 'TOOL_ERROR' });
      }
    });
  }
  ...
  return new Promise((resolve, reject) => {
    const server = app.listen(port, '0.0.0.0', () => {
      console.log(JSON.stringify({ event: 'health_check_started', data: { port } }));
      resolve(server);
    });
