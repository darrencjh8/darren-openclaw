# Feature Specification: Credit Card Statement Reconciliation
...
**Acceptance Criteria:**
- [ ] After content extraction, a lightweight LLM call (deepseek-v4-flash, no tools) classifies the email
- [ ] Classification input: Subject, From, and first 2000 characters of extracted body text
- [ ] Classification prompt: "Classify this email as 'statement' or 'transaction'."
- [ ] "statement" → Statement reconciliation pipeline (US-2 through US-6)
- [ ] "transaction" → Existing alert pipeline (unchanged)
- [ ] PDFs of single receipts are classified as "transaction" (not "statement")
- [ ] HTML tabular statement summaries are classified as "statement"
- [ ] Ambiguous input defaults to "transaction" (safe — goes through existing UC-2/3 guard logic)
