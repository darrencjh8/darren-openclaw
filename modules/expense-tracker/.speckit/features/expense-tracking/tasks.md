...
- [ ] Create `pyproject.toml` or `requirements.txt` with pinned dependencies:
...
- [ ] Create `src/__init__.py`, `src/agent/__init__.py`, `src/imap/__init__.py`, `src/extractors/__init__.py`, `src/client/__init__.py`, `src/notifier/__init__.py`, `src/utils/__init__.py`
- [ ] Create `tests/__init__.py`
- [ ] Create `.gitignore` (Python template + `.env` + `data/` + `__pycache__` + `*.pyc`)
- [ ] Create `config/` directory with placeholder `.gitkeep`
...
- [ ] Create `.env.example` with ALL variables from plan.md Section 9 (commented with descriptions)
...
- [ ] No third-party logging libraries; use stdlib `logging` + `json`
...
- [ ] Implement `src/extractors/html_extractor.py`:
...
- [ ] Implement `src/extractors/text_cleaner.py`:
...
- [ ] Implement `src/extractors/__init__.py` with `extract_email_content(msg: email.message.Message) -> str`:
...
- [ ] Implement `src/imap/idle_handler.py`:
...
- [ ] Write `tests/test_imap_handler.py`:
...
- [ ] Implement `src/notifier/email_notifier.py`:
  - `EmailNotifier` class:
...
- [ ] Tools that need the current email context receive it via a closure or context object
...
- [ ] No unit tests (content validation is manual review)
...
- [ ] Implement in `src/agent/orchestrator.py` (or separate `deepseek.py`):
...
- [ ] `src/__main__.py`: `from src.main import main; asyncio.run(main())`
...
Run after T0.1–T0.4:
- [ ] `python -c "import src; from src.config import Config; from src.utils.logging import setup_logging; from src.utils.dedup import DedupJournal"`
- [ ] `pytest tests/test_dedup.py -v` — all tests pass

### Checkpoint Bravo: Tools Complete
Run after T1.1–T1.5:
- [ ] `pytest tests/test_actual_client.py tests/test_extractors.py tests/test_imap_handler.py tests/test_notifier.py tests/test_tools.py -v` — all tests pass
- [ ] `python -c "from src.agent.tools import ToolRegistry; r = ToolRegistry(...); assert len(r.get_tool_schemas()) == 10"`

### Checkpoint Charlie: Agent Complete
Run after T2.1–T2.3:
- [ ] `pytest tests/test_agent_orchestrator.py -v` — all tests pass
- [ ] `python -c "from src.agent.prompts import SYSTEM_PROMPT; print(len(SYSTEM_PROMPT))"` — prompt is non-empty

### Checkpoint Delta: Integration Complete
Run after T3.1–T3.4:
- [ ] `pytest tests/ -v` — ALL tests pass (15+ tests)
- [ ] `docker build -f docker/Dockerfile .` — builds successfully
- [ ] Manual review: README.md is complete and accurate
- [ ] Manual review: `.env.example` has all variables from plan.md Section 9
