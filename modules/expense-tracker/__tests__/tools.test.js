...
describe("LLM tool schemas — budget_id in required", () => {
    let registry;
    ...
    // fetch_accounts, fetch_categories are the only AB tools exposed to Phase 1 LLM
    // (LLM_TOOLS = ["search_memory", "fetch_accounts", "fetch_categories"])

    test("fetch_accounts requires budget_id", () => {
        const s = findSchema("fetch_accounts");
        expect(s.function.parameters.required).toContain("budget_id");
        expect(
            s.function.parameters.properties.budget_id.default,
        ).toBeUndefined();
    });

    test("fetch_categories requires budget_id", () => {
        const s = findSchema("fetch_categories");
        expect(s.function.parameters.required).toContain("budget_id");
    });

    test("submit_decision requires budget_id", () => {
        const t = registry.getSubmitDecisionTool();
        expect(t.function.parameters.required).toContain("budget_id");
    });
});
