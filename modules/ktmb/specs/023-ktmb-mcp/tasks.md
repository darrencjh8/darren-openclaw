...
- **Action**: 
  - Remove all `from ktmb_core import ...` (zero exceptions)
  - Import lock helpers from `src.worker_lock`: `is_worker_running`, `check_stop_file`
  - Query logs via shared `src.utils.logging` circular buffer
  - Refactor `_handle_system_status` to call `is_worker_running()` and `check_stop_file()` instead of raw file I/O
- **Detail**: Verify all 12 `_handle_*` methods still compile and work. No behavior changes.
- **Effort**: 15 min
...
- **Tools**: Define all 12 `ktmb_*` tools as `@mcp.tool()` decorated async functions. Each calls `await registry.execute_tool(name, args_dict)`.
- **REST routes**: Reimplement all 12 `POST /tools/*` endpoints as Starlette HTTP routes (FastMCP is built on Starlette).
- **Pattern**:
  ```python
...
- **Detail**: `GET /health` endpoint preserved. All 12 `POST /tools/*` routes preserved as Starlette endpoints. `ToolRegistry` initialized once, shared between MCP and REST.
- **Effort**: 15 min
...
- **Action**: Add under `mcp_servers:`:
  ...
  ```
- **Detail**: No `transport` field needed — Hermes auto-detects streamable HTTP. Hermes auto-discovers all 12 tools.
- **Effort**: 5 min

### T011: Add Linux cron entry for worker

- **File**: Dockerfile or container entrypoint (`/etc/cron.d/ktmb-worker`)
- **Action**: Add cron entry that calls `ktmb_core.py` every 60 seconds:
  ```cron
  * * * * * root cd /app && python ktmb_core.py >> /var/log/ktmb-cron.log 2>&1
  ```
- **Detail**: Worker is NOT triggered via MCP or REST. No Hermes cron. `ktmb_core.py` handles lock acquisition internally. Notifies Hermes webhook on booking success/failure.
- **Effort**: 10 min
...
- **Expected**: All 12 `mcp_ktmb_booking_*` tools appear as available
...
| `ktmb_server.py` | ~5 (password reset: KTMB website + IMAP) |
