...
----
Session cache (persistent session backend, feat-001)
----

Before calling any provider, the middleware checks an encrypted SQLite
session cache. Cache hits skip the IDP round trip entirely. Cache misses
fall through to provider verification as before, and successful
verifications populate the cache. All cache operations are wrapped in
try/except — a broken store can never take down the auth gate.

The cache is a LOOKASIDE, not a source of truth. The IDP remains
authoritative.
"""
...
def _get_session_store():
    """Return the global session store, or None if not configured."""
    store = getattr(_get_session_store, "_instance", None)
    ...
    try:
        ...
        _get_session_store._instance = store
        return store
    except Exception:
        _log.warning("session store: failed to initialize, running without cache", exc_info=True)
        return None


async def _cache_session(session) -> None:
    """Best-effort: store a verified session in the cache."""
    try:
        from hermes_cli.dashboard_auth.session_store import StoredSession
        store = _get_session_store()
        if store is None:
            return
        await store.store(StoredSession.from_session(session))
    except Exception:
        _log.debug("session cache: _cache_session failed", exc_info=True)


async def _cache_load(at: str):
    """Try loading a cached session. Returns the Session object or None."""
    try:
        store = _get_session_store()
        if store is None:
            return None
        cached = await store.load(at)
        if cached is None:
            return None
        return cached.to_session()
    except Exception:
        _log.debug("session cache: _cache_load failed", exc_info=True)
        return None


async def _cache_delete(at: str) -> None:
    """Best-effort: remove a session from the cache."""
    try:
        store = _get_session_store()
        if store is None:
            return
        ...
        _log.debug("session cache: _cache_delete failed", exc_info=True)
...
async def gated_auth_middleware(
    ...
    session = None
    if at:
        # ── cache lookup first (feat-001) ──
        cached = await _cache_load(at)
        if cached is not None and cached.expires_at > int(__import__("time").time()):
            session = cached

        if session is None:
            # Cache miss or expired — fall through to provider verification.
            ...
            # ── cache after successful verify (feat-001) ──
            if session is not None:
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            # ── cache after successful refresh (feat-001) ──
            await _cache_session(new_session)
            return response
        ...
        # ── cache delete on session expiry (feat-001) ──
        if at:
            await _cache_delete(at)
        return response
