...
| 158 | B1: AB↔PP direction; dead `_abClient` | **CONFIRMED (code reality) / CODE-BUG (intent)** | `tools.js:376,382` `this._abClient = abClient` always `null`; never used. | Code does AB→PP; `_abClient` is dead. | Decision: remove dead field OR build AB-write. Doc → AB→PP. **Resolved (#261): dead `abClient` plumbing removed.** |
...
| 211 | #4 AB category env vars read but never consumed | **RESOLVED — removed** | Was: `config.js` set `abEmergencyPrimaryCategory`/`abEmergencySecondaryCategory`/`abWarchestCategory` from `AB_*_CATEGORY` with zero consumers (the issue's `ACTUAL_CATEGORY_*` names were stale). The assignments, the `.env.example` block, and all test assertions were removed. | Dead config, now gone | Done — verify with `grep -r "AB_EMERGENCY\|AB_WARCHEST"` |
...
| 218 | #16 `GOOGLE_SERVICE_ACCOUNT_JSON` fail-fast vs README "optional" | **CONFIRMED** | `index.js:33` guardEnv exits if missing; `tools.js:923-929` skips gracefully; `README.md:45` says "(optional)". | Currently mandatory at startup | Reconcile: make conditional OR fix README/spec. **Resolved (#261): docs say required, guard unchanged.** |
