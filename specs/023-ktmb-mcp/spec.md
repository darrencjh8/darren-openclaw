...
Replace the aiohttp REST server with a **FastMCP** server (Python `mcp` SDK). Exposes all 15 booking tools via **Streamable HTTP** transport (`POST /mcp`). Existing REST endpoints are preserved as plain Starlette HTTP routes (FastMCP is built on Starlette). The MCP server wraps the existing Python `ToolRegistry`.

**Key design constraints:**
- Tool registry refactored: NO imports from `ktmb_core` (except `reset_password`). Lock helpers extracted to shared `src/worker_lock.py`.
- Worker strictly singleton via PID-based lock file.
- Cron: 60 seconds via Hermes. Worker runs 3-4 poll cycles per trigger (`POLL_INTERVAL=15`, `MAX_RUNTIME=55`).
...
```mermaid
flowchart TB
    subgraph UC1["UC1: Book Train Ticket"]
        A1["👤 '/ktmb book JB→SG June 20 08:45'"] --> A2["ktmb_get_passenger()"] --> A3["ktmb_booking_window()"] --> A4["ktmb_validate_booking()"] --> A5["ktmb_create_booking()"] --> A6["📋 {job_id, watching}"] --> A7["⏰ 60s cron → 3-4 polls → 🔒 lock"] --> A8["🔄 Worker polls → books"] --> A9["📤 POST /webhooks/notify"] --> A10["📱 Telegram notification"]
    end

    subgraph UC2["UC2: Check / Cancel / Schedules"]
        B1["👤 '/ktmb status | cancel | schedules'"] --> B2["ktmb_list_orders / ktmb_order_status / ktmb_cancel_order / ktmb_get_schedules"] --> B3["📋 Result to user"]
    end

    subgraph UC3["UC3: System Management"]
        C1["👤 Admin"] --> C2["ktmb_system_status / pause / resume"] --> C3["ktmb_worker_logs"]
    end

    subgraph UC4["UC4: Password Reset"]
        D1["👤 Admin: '/ktmb reset-password'"] --> D2["ktmb_reset_password()"] --> D3["📧 Email → IMAP extract"] --> D4["🔄 Returns new password"]
    end

    subgraph UC5["UC5: Cron Worker Singleton"]
        E1["⏰ 60s cron"] --> E2["ktmb_trigger_worker()"] --> E3["🔒 acquire_lock()"] --> E4{"Lock?"}
        E4 -- "yes" --> E5["✅ 3-4 poll cycles → notify"] --> E6["🔓 release_lock()"]
        E4 -- "no" --> E7["⏭️ Skip (already running)"]
    end

    subgraph UC6["UC6: Save Passenger"]
        F1["👤 '/ktmb save-passenger'"] --> F2["ktmb_save_passenger()"] --> F3["📋 Profile persisted"]
    end
```
...
**Acceptance Criteria:**
- All 15 REST `POST /tools/*` endpoints still work (served as Starlette routes)
- Worker still triggered via `ktmb_trigger_worker` (REST or MCP), spawns daemon thread
...
| `ktmb_system_pause` | `{}` | `{paused, message}` |
| `ktmb_system_resume` | `{}` | `{resumed, message}` |
| `ktmb_worker_logs` | `{lines?, job_id?}` | `{logs[]}` |
| `ktmb_trigger_worker` | `{}` | `{triggered, message}` |
