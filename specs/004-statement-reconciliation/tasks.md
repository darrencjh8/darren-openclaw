...
- [ ] Add `IMAP_MAILBOX` to both `.env.example` files (ET: INBOX, PT: Trades)
- [ ] Add `imap_mailbox` field to both Config classes
- [ ] `deploy.sh`: Validate IMAP_MAILBOX as required

### T0.A3 — IMAP handlers use configured mailbox
- [ ] ET: `ImapIdleHandler` accepts mailbox param, `select(mailbox)` in connect
- [ ] PT: `EmailHandler` accepts mailbox param, `select(mailbox)` in connect
- [ ] PT: `mark_read` after processing trade emails

### T0.A4 — PDF notification workflow
- [ ] SKILL.md: Add missing-PDF workflow + pdf/PDF/attachment triggers
- [ ] Orchestrator: Expand email_trade message with no-PDF instructions
- [ ] Prompts: Add missing-PDF rule to SYSTEM_PROMPT
